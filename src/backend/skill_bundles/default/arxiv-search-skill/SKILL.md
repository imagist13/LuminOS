---
name: arxiv-search
display_name: arxiv 论文搜索与下载
description: 在 arxiv.org 上搜索并下载学术论文。当任务需要查找相关论文、进行文献综述或下载 PDF 以供分析时使用。涵盖 arxiv API 搜索、论文元数据获取、PDF 下载以及带速率限制保护的批量处理。专为自主研究工作流设计，让 Agent 能够自行检索文献。
author: LuminOS Team
version: 1.0.0
type: research
---
...
- 使用 `User-Agent` 请求头标明你的工具（例如 `LuminOS/1.0`）
...
```bash
curl -s -H "User-Agent: LuminOS/1.0" \
...
def search_arxiv(query: str, max_results: int = 10, start: int = 0):
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "LuminOS/1.0"})
...
```bash
curl -s -L -H "User-Agent: LuminOS/1.0" \
